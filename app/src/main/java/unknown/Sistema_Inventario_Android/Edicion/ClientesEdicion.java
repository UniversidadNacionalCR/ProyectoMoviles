...
public class ClientesEdicion extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    ...
    private void registrar() {
        ...
        Cursor c = db.rawQuery("SELECT "+ TablaCliente.ID_CLIENTES+" FROM "+ TablaCliente.TABLA_CLIENTE+" ORDER BY " + TablaCliente.ID_CLIENTES+ " asc",null);
        String Row="0";
        if (checkdb(c)) {
            c.moveToLast();
    ...
    private void promptDialogedit() {
        ...
        builder.setPositiveButton("Si", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SQLiteDatabase db=conn.getWritableDatabase();
                db.delete(TablaCliente.TABLA_CLIENTE, TablaCliente.CAMPO_NOMBRE + "='" + nombre+"'", null);
                db.close();
